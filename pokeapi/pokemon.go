...
func (c *Client)GetPokemonInfo(pokemonName string) (Pokemon, error) {
    ...
    if pokemonName == "" {
        return Pokemon{}, errors.New("Pokemon is empty")
    }
    endpoint := "pokemon/"
    full_url := baseURL + endpoint + pokemonName
    
    pokeData, ok := c.PokeApiCache.GetCacheVal(full_url)

    if ok {
        fmt.Println("cache hit")
        pokemonInfo := Pokemon{}
        err := json.Unmarshal(pokeData, &pokemonInfo)
        if err!=nil {
            return Pokemon{}, err
        }
    ...
    pokemonInfo := Pokemon{}
    error := json.Unmarshal(data, &pokemonInfo)
    if error!=nil {
